...
export function calculateTreeFit(svg_dim, tree_dim) {
  let k = Math.min(svg_dim.width *1.5/ tree_dim.width, svg_dim.height / tree_dim.height),
    x = tree_dim.x_off + (svg_dim.width*1.5 - tree_dim.width*k)/k/2,
...
export function mainToMiddle({datum, svg, svg_dim, scale, transition_time}) {
  const k = scale || 1, x = svg_dim.width*1.5/2-datum.x*k, y = svg_dim.height/2-datum.y,
    t = {k, x: x/k, y: y/k}
